...
class TestPhaseTracking:
    ...
    def test_placement_alone_does_not_terminate(self):
        """Placing the bottle in the well must NOT terminate the episode.
...
class TestCollisionDetection:
    ...
    def test_geom_sets_populated(self):
        """Bottle and crate geom sets should be non-empty for all reward types."""
        e = BottlePackingGymEnv(
            action_mode="abs_pos",
            reward_type="dense",
            well_index=0,
        ...
        try:
            assert len(e._bottle_geom_ids) > 0
            assert len(e._crate_geom_ids) > 0
        finally:
            e.close()
    ...
    def test_collision_flags_in_info(self, env):
        ...
        env.reset()
        _, _, _, _, info = env.step(env.action_space.sample())
        assert "collision_bottle_crate" in info
        assert "collision_bottle_bottle" in info
        assert isinstance(info["collision_bottle_crate"], bool)
        assert isinstance(info["collision_bottle_bottle"], bool)

    def test_no_bottle_bottle_collision_at_start(self, env):
        """Bottles should not be colliding with each other at episode start."""
        env.reset()
        _, _, _, _, info = env.step(env.action_space.sample())
        assert info["collision_bottle_bottle"] is False
