...
def main() -> None:
    ...
    # Determine if relative action key
    is_relative = args.action_key.endswith("_rel")
    is_10dof = "10dof" in args.action_key

    # Create environment, robot, and controller
    ...
    # Build T_initial for relative action reconstruction from the robot's
    # actual EE pose right after reset — matching generate_dataset.py which
    # captures initial_se3 before any physics steps run.
    T_initial = None
    if is_relative:
        T_initial = pos_rotmat_to_se3(robot.ee_pos, robot.ee_xmat)
        print(f"T_initial position: {T_initial[:3, 3]}")

    # Physics timestep for timing
    dt = env.model.opt.timestep
    step_time = dt * ACTION_REPEAT * args.slow
    ...
    for i in range(num_frames):
        ...
        # Extract gripper value (last element for both 8dof and 10dof)
        gripper_val = action[-1]
        robot.data.ctrl[7] = gripper_val * 255

        # Run IK + step physics ACTION_REPEAT times (matching generation loop
        # where controller.compute runs every physics step, not once per frame)
        ...
        # Print comparison
        ee_pos = robot.ee_pos
        err = np.linalg.norm(ee_pos - target_xyz)
        print(
