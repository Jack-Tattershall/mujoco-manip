...
# Conveyor belt speed (m per physics step) — 0.15 m/s at dt=0.002
CONVEYOR_SPEED = 0.0003

# Spacing between bottle centres on conveyor belt (metres)
CONVEYOR_BOTTLE_SPACING = 0.10
...
def well_position(well_index: int) -> np.ndarray:
    """Compute the 3D world position of a well center.
    ...
    x = CRATE_POS[0] + (col - (WELL_COLS - 1) / 2) * WELL_SPACING
    y = CRATE_POS[1] + (row - (WELL_ROWS - 1) / 2) * WELL_SPACING
    z = CRATE_POS[2]
